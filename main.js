...
var lyr_roads = new ol.layer.Tile({
   ...
   var layersList = [osmTile,lyr_adm1,lyr_adm2,lyr_adm3,lyr_landuse,lyr_roads,lyr_pois,lyr_places,lyr_raster];
...
switch(event.target.id) { 
    ...
    case "point": 
        ...
        interaction.on('drawend', function(event) {
            ...
            const id = prompt('Enter the ID of the point:');

            // Send the coordinates to the server using an HTTP POST request
    ...
    case "line":
        ...
        interaction.on('drawend', function(event) {
            ...
            const id = prompt('Enter the ID of the line:');
    ...
    case "polygon": 
        ...
        interaction.on('drawend', function(event) {
            ...
            const feature = event.feature;
            const coordinates = feature.getGeometry().getCoordinates();
            const id = prompt('Enter the ID of the polygon:');
            // Send the coordinates to the server to be saved in the database
...
// ***************************ADD POINT FROM DATABASE**************************************
// Client-side code to send the GET request and parse the point geometry
